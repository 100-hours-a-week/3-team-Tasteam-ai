...
사용 방법:
    python scripts/generate_ground_truth.py --input kr3.tsv --restaurants 2 --reviews-per-restaurant 50
"""
...
def read_kr3_tsv(file_path: str, num_restaurants: int = 2, reviews_per_restaurant: int = 50, seed: int = 42) -> Dict[int, List[Dict]]:
    ...
    Args:
        file_path: TSV 파일 경로
        num_restaurants: 생성할 레스토랑 수
        reviews_per_restaurant: 레스토랑당 리뷰 수
        seed: 랜덤 시드
        
    Returns:
        {restaurant_id: [reviews]} 딕셔너리
    """
    random.seed(seed)
    ...
    for rid, reviews in sorted(restaurants.items()):
        pos = sum(1 for r in reviews if r['rating'] == '1')
...
def generate_summary_ground_truth(restaurants: Dict[int, List[Dict]]) -> Dict:
    ...
    for restaurant_id, reviews in sorted(restaurants.items()):
        ...
        # 간단한 aspect 추출 (실제로는 수동 검수 필요)
        # 맛, 양, 서비스, 가격, 분위기 등 키워드 기반으로 추출
        positive_aspects = []
        negative_aspects = []
        ...
        # overall_summary 생성 (aspect 기반)
        summary_parts = []
        if positive_aspects:
            aspect_names = [a['aspect'] for a in positive_aspects]
            summary_parts.append(f"{', '.join(set(aspect_names))}에 대한 긍정 언급이 많고")
        if negative_aspects:
            aspect_names = [a['aspect'] for a in negative_aspects]
            summary_parts.append(f"{', '.join(set(aspect_names))} 관련 불만도 있습니다")
...
def generate_strength_ground_truth(restaurants: Dict[int, List[Dict]]) -> Dict:
    ...
    for restaurant_id, reviews in sorted(restaurants.items()):
        ...
        if taste_reviews and len(taste_reviews) >= 3:
            ...
            # distinct 버전 (distinct_score 추가)
            distinct_strength = strength.copy()
            distinct_strength["type"] = "distinct"
            distinct_strength["distinct_score"] = 0.85  # 샘플 값 (실제로는 비교군 기반 계산 필요)
...
def generate_vector_search_ground_truth(restaurants: Dict[int, List[Dict]]) -> Dict:
    ...
    for restaurant_id, reviews in sorted(restaurants.items()):
        ...
        # 가격 쿼리
        price_reviews = [r for r in reviews if '가격' in r['content'] or '비싸' in r['content'] or '저렴' in r['content']]
        if price_reviews:
            result["queries"].append({
                "query": "가격 합리적",
                "restaurant_id": restaurant_id,
...
def main():
    ...
    print("   ⚠️  주의: overall_summary와 aspect는 수동 검수 및 수정이 필요합니다.")
    ...
    print("   ⚠️  주의: relevant_review_ids는 수동 검수 및 정렬이 필요합니다.")
