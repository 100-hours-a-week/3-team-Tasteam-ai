...
실행:
  ...
  python scripts/distill_flows.py evaluate_on_pod --adapter-path .../adapter --val-labeled-path .../val_labeled.json [--test-labeled-path ...]  # Pod: distill_summary no-evidence 추론 + 로컬 judge v2_no_evidence (기존 evidence는 --legacy-evidence-eval)
...
def _eval_llm_judge_cli_extras(full_no_evidence: bool) -> list[str]:
    """eval_llm_as_judge.py: distill_summary 추론 트랙과 맞출 때 v2_no_evidence + prediction 스트립."""
    if full_no_evidence:
        return ["--rubric-version", "v2_no_evidence", "--prediction-no-evidence"]
    return []
...
def evaluate_task(
    ...
    if sample_ids and val_labeled_path and Path(val_labeled_path).exists():
        ...
        cmd_judge.extend(_eval_llm_judge_cli_extras(full_no_evidence_eval))
...
def evaluate_on_pod_task(
    ...
    full_no_evidence_eval=True(기본): distill_summary 프롬프트·후처리 + eval_llm_as_judge v2_no_evidence.
    """
    if RunPodClient is None:
        raise RuntimeError("RunPodClient not available.")
...
def download_eval_from_volume_and_finish_task(
    ...
    sample_ids = report_data.get("meta", {}).get("llm_judge_sample_ids", [])
    if sample_ids:
        judge_no_ev = bool(report_data.get("meta", {}).get("prediction_no_evidence", False))
        cmd_judge = [
            sys.executable,
            str(_SCRIPT_DIR / "eval_llm_as_judge.py"),
            "--report", str(report_path),
        ...
        cmd_judge.extend(_eval_llm_judge_cli_extras(judge_no_ev))
        rj = subprocess.run(cmd_judge, cwd=str(_PROJECT_ROOT), capture_output=True, text=True)
        if rj.returncode != 0:
            logger.warning("eval_llm_as_judge failed: %s", rj.stderr or rj.stdout)
        elif llm_judge_path.exists():
