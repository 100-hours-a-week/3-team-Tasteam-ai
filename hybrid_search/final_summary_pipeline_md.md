...
대표 벡터(centroid) top_k 리뷰에서 llm이 긍정 리뷰와 부정 리뷰를 뽑고, 그 aspect의 claim이 진짜 리뷰 목록 맞는지에 대해 cos 검증을 수행해서, 통과할 경우에만, 그 리뷰를 사용 했음. llm은 긍정 2개, 부정 1개를 갖고오게함. 그 개수를 충족할 때까지 검증, 재시도를 수행. 재시도가 전부 실패하면 그냥 확보한 것만 가지고감.
