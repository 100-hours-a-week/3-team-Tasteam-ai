...
> RunPod Network Volume을
> AWS S3처럼 접근할 수 있게 해주는 기능 설명서
...
```
로컬 → S3 API → Network Volume
```

---

# 🔷 3. Datacenter & Endpoint

각 데이터센터마다 S3 API 주소가 다름:

예:

| Datacenter | Endpoint                                                           |
| ---------- | ------------------------------------------------------------------ |
| EU-RO-1    | [https://s3api-eu-ro-1.runpod.io](https://s3api-eu-ro-1.runpod.io) |
...
⚠️ 환경변수에 기존 AWS 키가 있으면 덮어씌워질 수 있음.
...
| Pod 안 경로         | S3 API 경로            |
| ---------------- | -------------------- |
| /workspace/a.txt | s3://VOLUME_ID/a.txt |
...
자동 multipart upload 됨

### 10GB 이상

timeout 가능 → timeout 설정 늘려야 함
...
> Network Volume을 “객체 저장소처럼” 다루게 해주는 것
...
> RunPod Network Volume을 AWS S3처럼 접근할 수 있게 만든 인터페이스이며,
> Pod를 켜지 않고도 파일을 관리할 수 있다.