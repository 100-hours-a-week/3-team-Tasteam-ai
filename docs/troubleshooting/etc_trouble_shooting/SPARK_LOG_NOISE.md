...
| **Spark 내부 로그 제거** | `logger.spark.name=org.apache.spark`, `logger.spark.level=off` (TaskSetManager/BlockManager/Executor 등) |

- `src/comparison_pipeline.py`: `_get_spark()`에서 위 설정 적용

### log4j2 설정 후에도 스파크 로그가 많이 뜨는 경우

- **원인**: `org.apache.spark` 로거를 낮추지 않으면 TaskSetManager, BlockManager, Executor 등에서 나오는 WARN/ERROR(예: PYTHON_VERSION_MISMATCH 스택)가 그대로 출력됨.
- **조치**: `conf/log4j2-spark-quiet.properties`에 `logger.spark.name = org.apache.spark`, `logger.spark.level = off` 추가. 파일 문법은 Spark의 `conf/log4j2.properties.template`와 동일한 Log4j 2 properties 형식이어야 함.