...
파이프라인 동작·흐름·Config는 **docs/architecture/ARCHITECTURE_OVERVIEW.md** 를 참고하세요.

- **정의 위치**: `src/models.py`, 배치 큐 DTO는 `src/api/routers/batch.py` 내부
- **라우터**: `src/api/routers/llm.py`, `sentiment.py`, `vector.py`, `batch.py`
...
| [2. 엔드포인트·DTO 요약](#2-엔드포인트dto-요약) | API별 Request/Response DTO 링크 (배치 큐 포함) |
...
| POST | `/api/v1/batch/enqueue` | BatchEnqueueRequest | BatchEnqueueResponse |
| GET | `/api/v1/batch/status/{job_id}` | - | BatchStatusResponse |

**배치 큐**: `BATCH_USE_QUEUE=true` 및 Redis 연결 시에만 enqueue/status 동작. 비활성 시 503.
...
- **Vector (업로드만)**: [4.4](#44-vector), [5.5](#55-vector-dto)
- **Batch (큐)**: [4.5](#45-batch-큐), [5.9](#59-batch-큐-dto)
...
**현재 제공 API**: `POST /api/v1/vector/upload` 만 제공. 유사 리뷰 검색(`/vector/search/similar`)은 제거되었으며, Summary/Sentiment/Comparison은 내부에서 `VectorSearch`(하이브리드 검색·리뷰 조회)를 사용합니다.
...
// 요청 (BatchEnqueueRequest)
{
  ...
  "limit": 10                 // summary용. 선택, 기본 10
}

// 응답 (BatchEnqueueResponse)
{
  "job_id": "abc-123-def",
  "job_type": "sentiment",
  "queue": "batch"
...
- **역할**: 작업 상태 조회. RQ `get_status()` 기반.

```json
// 응답 (BatchStatusResponse)
{
  "job_id": "abc-123-def",
  "status": "finished",      // queued | started | finished | failed | deferred
  "result": "...",           // 완료 시 결과 JSON 문자열(최대 2000자)
  "exc_info": null           // 실패 시 예외 정보
}
```
...
API별 요청/응답에 사용되는 Pydantic 모델(DTO)과 필드를 정리합니다. 정의 위치: `src/models.py`, 배치 큐 DTO는 `src/api/routers/batch.py`.  
*(라우터: `src/api/routers/llm.py`, `sentiment.py`, `vector.py`, `batch.py` 기준으로 검증됨.)*
...
**참고**: 벡터 유사 검색 API(`/vector/search/similar`)는 제거됨. `VectorSearchRequest`, `VectorSearchResult`, `VectorSearchResponse`는 `src/models.py`에만 존재하며 외부 API로는 제공되지 않음. 아래는 **업로드 API**에 사용되는 DTO만 기술.
...
정의 위치: `src/api/routers/batch.py`
...
| | job_type | str | 요청한 job_type |
| | queue | str | 사용된 RQ 큐 이름 (Config.RQ_QUEUE_NAME) |
| **BatchStatusResponse** | job_id | str | 조회한 job ID |
| | status | str | `queued` \| `started` \| `finished` \| `failed` \| `deferred` |
| | result | str? | 완료 시 결과 JSON 문자열(최대 2000자 절단) |
| | exc_info | str? | 실패 시 예외 정보(최대 2000자 절단) |
...
파이프라인 동작·흐름·Config는 **docs/architecture/ARCHITECTURE_OVERVIEW.md** 를 참고하세요.  
오프라인 배치 전략(batch-runner, RQ 워커, 재시도·DLQ)은 **docs/batch/offline_batch_strategy.md** 를 참고하세요.
