...
- 프롬프트: eval과 동일 (system + few-shot + instruction).
  no_evidence_prompt=True 시 SCHEMA_ENFORCEMENT_SYSTEM_NO_EVIDENCE + few-shot (evidence 필드 없음).
- 추론: base + LoRA adapter, apply_chat_template + generate(do_sample=False, max_new_tokens=1024).
- 후처리: extract_json_for_rouge → postprocess_prediction 또는 postprocess_prediction_no_evidence.
- evidence 필드 제거(_drop_evidence_fields): no_evidence_output=True 일 때만 (no_evidence_prompt와 무관).
"""
...
# evidence 필드를 요구하지 않는 요약 트랙 (LLM-as-judge v2_no_evidence 등)
SCHEMA_ENFORCEMENT_SYSTEM_NO_EVIDENCE = """당신은 리뷰 요약 어시스턴트입니다.
입력과 출력은 항상 JSON 형식이다.
다음은 입력과 출력의 JSON 스키마이다.
...
출력 JSON 스키마 (evidence 필드 없음):
{
  "service": {"summary": string, "bullets": [string, ...]},
  "price":   {"summary": string, "bullets": [string, ...]},
  "food":    {"summary": string, "bullets": [string, ...]},
  "overall_summary": {"summary": string}
}
...
출력 JSON 스키마 설명
- summary: 해당 카테고리 입력 리뷰들의 총 요약문. bullets: 해당 카테고리 입력 리뷰들의 요소별 요약문.
- overall_summary에는 summary만 있고 bullets 없음.
- evidence 필드를 출력하지 말 것.
...
TINY_FEWSHOT_ASSISTANT_NO_EVIDENCE = """
예시 출력:
{"service":{"summary":"직원들이 친절하고 응대가 만족스러워요.","bullets":["점심시간에 대기가 있지만 직원들이 친절해요.","음식이 비교적 빨리 나와서 만족스러워요.","매장이 쾌적하다고 해요.","직원분들이 친절하다고 언급해요."]},"price":{"summary":"양이 많아서 만족스럽다는 의견이 있어요.","bullets":["양이 많다고 해요."]},"food":{"summary":"음식이 맛있고 자주 방문하고 싶어요.","bullets":["쌀국수가 특히 맛있다고 해요.","음식이 전반적으로 맛있다고 해요.","팀원들과 자주 방문하게 된다고 해요.","점심으로 먹기 좋다고 해요."]},"overall_summary":{"summary":"전반적으로 서비스가 친절하고 음식 만족도가 높아요."}}
"""
...
def postprocess_prediction_no_evidence(pred_json_str: str, instruction: str) -> str:
    ...
    for cat in ("service", "price", "food"):
        ...
        pred[cat] = {"summary": summary.strip(), "bullets": bullets}
...
def generate_one(
    ...
    postprocess=True면 후처리까지 적용.
    no_evidence_prompt=True면 evidence 없는 스키마 프롬프트·few-shot만 바꿈 + (postprocess 시) postprocess_prediction_no_evidence.
    evidence 필드 제거는 no_evidence_output=True 일 때만 수행 (no_evidence_prompt와 독립).
    """
...
def generate_summary_from_payload(
    ...
    """
    payload(service/price/food 리스트)로 instruction 문자열을 만든 뒤 추론하여
    summary 구조(dict)로 반환. API summary 파이프라인에서 사용.
    운영 기본은 eval no-evidence 트랙과 정합:
    - postprocess=False
    - no_evidence_prompt=True
    - no_evidence_output 는 호출 인자로 제어
    """
